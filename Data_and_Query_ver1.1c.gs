...
function getOfflineAnswers(question) {
  ...
  var entryRange = dash.getRange("A11");
  var statusCell = dash.getRange("A16");
  var answerArea = dash.getRange("A19");
  ...
  // OPTIONAL: if you have leftover "irregulars" var from older code, keep consistent:
  // var irregulars = irregMap;   // uncomment if other parts of code use 'irregulars'
  ...
  // Keep top 10
  var keywords = filtered.slice(0, 10);
  ...
  for (var i = 0; i < data.length; i++) {
    ...
    for (var k = 0; k < keywords.length; k++) {
      ...
      debugHits.push("TRY:" + term + " => " + (hit ? "MATCH" : "no"));
        if (hit) {
          familyMatched = true;
      ...
      if (familyMatched) {
        ...
        debugHits.push("MATCHED_FAMILY:" + kw + " via: " + matchedTerms.join(", "));
      } else {
      // helpful: record the whole family when nothing matched
        debugHits.push("NO_MATCH_FAMILY:" + kw + " family=[" + Array.from(family).join(", ") + "]");
      }
    }    
...
function loadIrregulars() {
  var ss = SpreadsheetApp.getActiveSpreadsheet();
  var sheet = ss.getSheetByName("IrregularVerbs");
  if (!sheet) return {};

  var data = sheet.getDataRange().getValues();
  var irregulars = {};

  // Assume header row: Base Verb | Past | Past Participle | Present Participle
  for (var i = 1; i < data.length; i++) {
    var row = data[i];
    var base = row[0];
    if (!base) continue;

    irregulars[base.toLowerCase()] = row
      .slice(0, 4) // base + 3 forms
      .filter(Boolean) // remove blanks
      .map(x => x.toLowerCase().trim()); // normalize
  }
